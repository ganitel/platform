...
export default function ReviewInformation() {
  ...
  const calculateNights = () => {
    ...
    return bookingData?.nights || 1;
  };

  const nights = calculateNights();
  const pricePerNight = propertyData?.pricing?.base_price || propertyData?.price_per_night || propertyData?.price || 0;
  const subtotal = nights * pricePerNight;
  
  // Use provided fees or default to 0
  const serviceFee = propertyData?.service_fee || 0;
  const cleaningFee = propertyData?.cleaning_fee || 0;
  const taxes = propertyData?.taxes || 0;
  ...
  return (
    <div className="min-h-screen bg-ganitel-neutral-1 flex flex-col md:w-[360px] mx-auto pb-32">
      ...
      <div className="bg-white px-4 py-3 flex items-center justify-between">
        ...
        <h2 className="text-xl font-extrabold text-[#18100C] absolute left-1/2 transform -translate-x-1/2">Review informations</h2>
        ...
        <div className="bg-white rounded-xl p-4 space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span style={{ color: '#67615F' }}>${pricePerNight.toLocaleString()} × {nights} night{nights !== 1 ? 's' : ''}</span>
              <span className="text-gray-900 font-medium">${subtotal.toLocaleString()}</span>
            </div>
            {serviceFee > 0 ? (
              <div className="flex justify-between text-sm">
                <span style={{ color: '#67615F' }}>Service fee</span>
                <span className="text-gray-900 font-medium">${serviceFee.toLocaleString()}</span>
              </div>
            ) : (
              <div className="flex justify-between text-sm">
                <span style={{ color: '#67615F' }}>Service fee</span>
            ...
            {cleaningFee > 0 ? (
              <div className="flex justify-between text-sm">
                <span style={{ color: '#67615F' }}>Cleaning fee</span>
                <span className="text-gray-900 font-medium">${cleaningFee.toLocaleString()}</span>
              </div>
            ) : (
              <div className="flex justify-between text-sm">
                <span style={{ color: '#67615F' }}>Cleaning fee</span>
            ...
            {taxes > 0 ? (
              <div className="flex justify-between text-sm">
                <span style={{ color: '#67615F' }}>Taxes</span>
                <span className="text-gray-900 font-medium">${taxes.toLocaleString()}</span>
              </div>
            ) : (
              <div className="flex justify-between text-sm">
                <span style={{ color: '#67615F' }}>Taxes</span>
        ...
        <div className="bg-white rounded-[24px] p-6 shadow-sm border border-gray-50 space-y-6">
          ...
          <div className="space-y-5">
            <div>
              ...
              <p className="text-[16px] font-extrabold text-[#18100C]">{booking?.travelerInfo?.firstName || travelerData?.firstName || "Gédéon"}</p>
            </div>

            <div>
              <p className="text-sm font-medium text-gray-400 mb-1">Name</p>
              <p className="text-[16px] font-extrabold text-[#18100C]">{booking?.travelerInfo?.lastName || travelerData?.lastName || "Atcheng"}</p>
            </div>

            <div>
              <p className="text-sm font-medium text-gray-400 mb-1">Email</p>
              <p className="text-[16px] font-extrabold text-[#18100C]">{booking?.travelerInfo?.email || travelerData?.email || "user@gmail.com"}</p>
            </div>

            <div>
              <p className="text-sm font-medium text-gray-400 mb-1">WhatsApp Number</p>
              <p className="text-[16px] font-extrabold text-[#18100C] tracking-wide">
                {booking?.travelerInfo?.phone || travelerData?.fullPhoneNumber || "+33 1 09 75 83 51"}
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Footer Button - Part of flow for consistency or fixed? */}
      {/* Screenshot shows it at the bottom, we'll put it at the end of scrollable to match previous fix or keep fixed */}
      <div className="p-4 px-6 md:max-w-[360px] md:mx-auto">
        <Button
