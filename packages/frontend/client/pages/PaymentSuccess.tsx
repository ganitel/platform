...
export default function PaymentSuccess() {
  ...
  const extraCharges = serviceFee + cleaningFee + taxes;
  const total = bookingData?.pricing?.total || (subtotal + extraCharges);

  const totalGuests = booking?.guests 
    ? (booking.guests.adults || 0) + (booking.guests.children || 0) + (booking.guests.infants || 0)
    : 4;

  const hostName = propertyData?.host?.name || "M. Jacques Zeh";
  const hostPhone = propertyData?.host?.phone || "+237 6 59 47 80 87";
  const hostEmail = propertyData?.host?.email || "Jacques.zeh@gmail.com";
  const hostRating = propertyData?.host?.rating || 5;
  const hostReviews = propertyData?.host?.reviews || 693;
  ...
  const getCancellationDeadline = () => {
    if (booking?.checkIn) {
      ...
      return cancellationDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
    }
    return "Jul 30, 2025";
  };
  ...
  const formattedDates = () => {
    ...
    return "March 15-20, 2024";
  };

  return (
    <div className="min-h-screen bg-ganitel-neutral-1 flex flex-col md:w-[360px] mx-auto pb-12">
      {/* Header */}
      <div className="bg-white px-4 py-4 flex items-center justify-between sticky top-0 z-40">
        <button onClick={() => navigate("/")} className="p-0 w-6 h-6 flex items-center justify-center">
          <img src="/icons/Menu, Burger, Square.svg" alt="Home" className="w-6 h-6" />
        </button>
        ...
        <button className="p-0 w-6 h-6 flex items-center justify-center">
          <img src="/icons/User, Circle.png" alt="User" className="w-6 h-6" />
        </button>
      </div>
      ...
      <div className="flex-1 px-4 py-6 space-y-4">
        ...
        <div className="text-center space-y-3 py-2">
          <div className="flex justify-center text-[80px]">🎊</div>
          <div className="inline-flex items-center bg-white border border-black px-4 py-1.5 rounded-xl text-sm font-bold">
            Payment completed
          </div>
        </div>
        ...
        <div className="bg-white rounded-[24px] p-6 shadow-sm border border-gray-50 space-y-4">
          ...
          <div className="space-y-2 text-gray-500 font-bold">
            <div className="flex justify-between items-center">
              <span>Subtotal: $ {subtotal.toLocaleString()}</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <span>Extra: $ {extraCharges.toLocaleString()}</span>
            </div>
          </div>
          <div className="flex items-center gap-3 pt-2">
            <Receipt className="w-6 h-6 text-[#18100C]" />
            <span className="text-xl font-extrabold text-[#18100C]">Total: $ {total.toLocaleString()}</span>
          </div>
        </div>

        {/* Apartment Card */}
        <div className="bg-white rounded-[24px] p-6 shadow-sm border border-gray-50 space-y-1">
          <h2 className="text-[17px] font-extrabold text-[#18100C] capitalize">
            {propertyData?.type || "Apartment"}
          </h2>
          <p className="text-gray-400 font-bold">
            {formattedDates()} • {totalGuests} Guests
          </p>
        </div>

        {/* Contact Host Card */}
        <div className="bg-white rounded-[24px] p-6 shadow-sm border border-gray-50 space-y-6">
          <h2 className="text-[17px] font-extrabold text-[#18100C]">Contact your Host</h2>
          
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-gray-100 rounded-2xl overflow-hidden flex-shrink-0">
               {propertyData?.host?.image ? (
                 <img src={propertyData.host.image} alt="Host" className="w-full h-full object-cover" />
               ) : (
                 <div className="w-full h-full flex items-center justify-center bg-[#FAD9BD] text-[#18100C] font-bold text-xl uppercase">
                   {hostName.charAt(0)}
                 </div>
               )}
            </div>
            <div>
              <p className="font-extrabold text-[#18100C] text-lg">{hostName}</p>
              <div className="flex items-center gap-1">
                <Star className="w-3.5 h-3.5 text-amber-500 fill-amber-500" />
                <span className="text-sm font-bold text-[#18100C]">{hostRating}</span>
                <span className="text-sm text-gray-400 font-bold">({hostReviews})</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 pt-2">
            <div className="flex items-center gap-4">
              <div className="bg-gray-50 p-2.5 rounded-xl">
                <Phone className="w-5 h-5 text-[#18100C]" />
              </div>
              <span className="font-bold text-[#18100C]">{hostPhone}</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="bg-gray-50 p-2.5 rounded-xl">
                <Mail className="w-5 h-5 text-[#18100C]" />
              </div>
              <span className="font-bold text-[#18100C] break-all">{hostEmail}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[24px] p-6 shadow-sm border border-gray-50 space-y-4">
          ...
          <div className="bg-[#F6F5F5] rounded-2xl p-6">
            <p className="text-[15px] font-bold text-gray-400">
              Free cancellation until <span className="text-[#18100C] font-extrabold">{getCancellationDeadline()}</span>
            </p>
            <p className="text-[15px] font-bold text-gray-400 mt-1">
