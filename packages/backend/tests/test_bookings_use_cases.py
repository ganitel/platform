...
class TestCreateBookingUseCase:
    ...
    def test_create_booking_exceeds_max_guests(
        self,
        booking_repository,
        service_repository,
        user_repository,
    ...
    def test_create_booking_date_conflict(
        self,
        booking_repository,
        service_repository,
        user_repository,
...
class TestConfirmBookingUseCase:
    ...
    def test_confirm_booking_success(self, booking_repository, sample_booking):
        ...
        confirmed_booking = use_case.execute(booking_id=sample_booking.id)
    ...
    def test_confirm_booking_not_pending(self, booking_repository, sample_booking):
        ...
        with pytest.raises(
            ...
            use_case.execute(booking_id=sample_booking.id)
    ...
    def test_confirm_booking_not_found(self, booking_repository):
        ...
        with pytest.raises(BookingNotFoundError):
            use_case.execute(booking_id=uuid4())
...
class TestCompleteBookingUseCase:
    ...
    def test_complete_booking_success(self, booking_repository, sample_booking):
        ...
        completed_booking = use_case.execute(booking_id=sample_booking.id)
    ...
    def test_complete_booking_not_confirmed(self, booking_repository, sample_booking):
        ...
        with pytest.raises(
            ValidationError, match="Only confirmed bookings can be completed"
        ):
            use_case.execute(booking_id=sample_booking.id)
...
class TestCancelBookingUseCase:
    ...
    def test_cancel_booking_unauthorized(
        self, booking_repository, sample_booking, sample_provider
    ):
    ...
    def test_cancel_booking_admin_authorized(
        self, booking_repository, sample_booking, sample_admin
    ):
        """Test admin can cancel any booking"""
        use_case = CancelBookingUseCase(booking_repository)

        cancelled_booking = use_case.execute(
            booking_id=sample_booking.id, requester_id=sample_admin.id, is_admin=True
        )
