...
def test_global_ganitel_exception_handler_returns_standard_payload(client):
    """GanitelException should be formatted by global handler with error_code and request_id."""
    path = "/api/v1/_test/ganitel-exception"

    async def _raise_ganitel_error():
        raise GanitelException("Simulated business rule failure", status_code=418)
    ...
    assert payload["error_code"] == "GanitelException"
...
def test_global_unhandled_exception_handler_hides_internal_details(client):
    ...
    with TestClient(app, raise_server_exceptions=False) as safe_client:
        response = safe_client.get(path, headers={"X-Request-ID": "req-unhandled-handler-1"})
...
def test_payment_initiation_500_is_generic_without_internal_exception(client, auth_token, sample_booking, monkeypatch):
    ...
    monkeypatch.setattr(payments_module.InitiatePaymentUseCase, "execute", _broken_execute)
...
def test_admin_stats_500_is_generic_without_internal_exception(client, admin_token, monkeypatch):
    ...
    monkeypatch.setattr(admin_module.GetDashboardStatsUseCase, "execute", _broken_execute)
