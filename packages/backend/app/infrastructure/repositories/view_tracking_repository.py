...
class ViewTrackingRepository(IViewTrackingRepository):
    ...
    def get_by_id(self, view_id: UUID) -> ViewTracking | None:
    ...
    def get_view_count(self, entity_type: str, entity_id: UUID) -> int:
        ...
        return (
            self.db.query(func.count(ViewTracking.id))
            .filter(
                ViewTracking.entity_type == entity_type,
                ViewTracking.entity_id == entity_id,
            )
    ...
    def get_by_user_id(
        ...
        return (
            self.db.query(ViewTracking)
            .filter(ViewTracking.user_id == user_id)
            .order_by(ViewTracking.created_at.desc())
            .offset(skip)
            .limit(limit)
            .all()
